...
class LinienDataHandler:
    ...
    LOG_FILE = Path(__file__).parent / "linien_data_handler.log" #creates the log file

    def __init__(self): #each time a class object is created it automatically initialize these object attributes
    ...
    def _load_reference_lines(self):
        """
        Load all D2 reference lines from the REFERENCE_LINES_FOLDER directory into a dictionary formatted as {"x", "y", "V_lock_start", "V_lock_end"}. For each line there must be:
