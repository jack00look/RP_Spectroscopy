...
class AutolockAlgorithmSelector:
    ...
    def __init__(
        ...
        if mode_preference != AutolockMode.AUTO_DETECT:
            logger.debug("_JJ_ autolock mode is not autodetect")
            self.mode = mode_preference
            self.done = True
            return
        logger.debug("_JJ_ autolock mode is autodetect")
        self.check()
